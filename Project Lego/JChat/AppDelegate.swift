        ...
        service.request(.search(lat: coordinate.latitude, long: coordinate.longitude)) { [weak self] (result) in
            ...
            case .success(let response):
                ...
                if let nav = strongSelf.window!.rootViewController as? UINavigationController,
                    let eventListViewController = nav.topViewController as? EventTableViewController {
                    eventListViewController.viewModels = viewModels ?? []
                    ...
                    strongSelf.window!.rootViewController?.present(nav, animated: true) {
                        (nav.topViewController as? EventTableViewController)?.delegete = self as! ListActions
                        (nav.topViewController as? EventTableViewController)?.viewModels = viewModels ?? []
                    }
                }
            case .failure(let error):
                print("Error: \(error)")
